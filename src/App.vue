...
<template>
  <div class="p-4">
    ...
    <div class="w-full overflow-x-auto">
      <table class="w-full">
          <tbody class="bg-white space-y-2 w-full">
           <tr v-for="(app,index) in appointments" :key="app.id" :class="{'bg-gray-100': index % 2 === 0}" class=" cursor-pointer border border-gray-200 rounded-lg flex shadow-sm hover:shadow py-4">
              <td class="py-3 px-4 flex items-center justify-center flex-1 ">
                  <div class="flex flex-col gap-y-2.5 flex-1 items-start pl-8">
              ...
              <td class="py-3 px-4 flex items-center justify-center flex-1 ">
                  <div class="flex items-center justify-between gap-x-2.5 bg-pink-500 p-2 rounded-full flex-1 pl-8">
                        <div class="bg-white  py-1 px-3 text-sm rounded-full flex items-center justify-between gap-x-3">
                          <span :class="getStatusColor(appointmentStatus(app.fields.is_cancelled !== undefined ? app.fields.is_cancelled : undefined, app.fields.appointment_date))">{{ appointmentStatus(app.fields.is_cancelled !== undefined ? app.fields.is_cancelled : undefined, app.fields.appointment_date) }}</span>
                          <span v-if="appointmentStatus(app.fields.is_cancelled !== undefined ? app.fields.is_cancelled : undefined, app.fields.appointment_date) === 'Upcoming'" class="">{{ getTimeRemaining(app.fields.is_cancelled !== undefined ? app.fields.is_cancelled : undefined, app.fields.appointment_date) }}</span>
                        </div>
                        <div class="text-white pr-4 flex items-center justify-between gap-x-2">
                          <span>
...
import { User, Mail, Phone, House, Clock } from 'lucide-vue-next';
...
const getAllAppointments = async () => {
  ...
  if(res.status === 200){
    appointments.value = res.data.records.sort((a, b) => {
